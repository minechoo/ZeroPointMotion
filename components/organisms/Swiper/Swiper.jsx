	...
	return (
		<figure className={clsx(styles.visual)}>
			...
			<Text className={clsx(styles.txt_ex)}>
				...
				<br />
				{`여기에는
작품<안드로지니 (Androgyny)>은 성(性)에 대한 다각적인 질문에서 출발하는 토크방식의 게임퍼포먼스이다.

‘안드로귀노스’ 혹은 ‘안드로지니(androgyny), 안드로진(androgine)은 플라톤의 향연에 등장하여 그리스의 인간 창조 이야기를 전개하는 핵심 용어이다.

‘안드로귀노스’는 그리스 신화가 설명하는 태고의 인간으로, 이 신화를 따르면 인간은 본래 남자와 여자의 양성이 한 몸을 이룬 자웅동체였다. 그러나 그는 제우스신의 진노를 받아 남자와 여자로 분리되며, 결혼을 통해 예전에 잃어버린 본래의 짝을 찾는다. 이러한 헬레니즘적 신화는 구약성서를 기반으로하는 히브리즘에는 정면 대치된다. 야훼는 남.녀를 각각 따로 창조했기 때문이다. 이런 종교관은 남성권위주의를 의심 없이 정당화하기에 현대에 이르러 페미니즘과 지속적인 충돌을 낳게 되는 근본 원인일 수 있다`}
				<br />
				<br />

				{`신화에서 페미니즘까지.

이 신화적 이야기를 패러디로 변형된 안드로지니를 성(性)에 대한 잡다한 신화적, 종교적, 문학적, 생물학적, 심리적 담론으로까지 발전시켜 본다. 하지만 여기서 멈추지 않고 또 다른 담론까지 끌어들인다. AI에 의해 기계화되는 미래. 인공수정을 넘어 영화 매트릭스에서 보듯이 기계에서 생명체를 부양한다면 그리고 다시 양성의 인간이 출현한다면?`}
			</Text>
		</figure>
	);
}
